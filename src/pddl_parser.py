...
cwd = os.getcwd()
if 'src' in cwd:
    pyper_plan_loaction = os.path.join('../parsers/pyperplan_s-ex')
...
def test_generate_planning_files():
    ...
    for cur_action in planning_domain.actions:
        print(planning_domain.actions[cur_action].name)
        print(planning_domain.actions[cur_action].precondition)
        
        #print(action.signature[0])
    ...
    problem = pddl_parser.parse_problem(planning_domain)
    print(problem)

def parse_pddl_files(domain_file_name, problem_file_name):
    cur_parser = parser.Parser(domain_file_name, problem_file_name)
    planning_domain = cur_parser.parse_domain()
    problem = cur_parser.parse_problem(planning_domain)
    #print(planning_domain)
    #print(problem)
    import grounding
    # perform the grounding.
    # first False is to cancel relevance analyis
    # second False is to cancel static analysis in the pyperplan's groundin.py ground_action 
    planning_task = grounding.ground(problem,False, False)
    #print('planning task')
    #print(planning_task)
    return[planning_domain,problem,planning_task]
    
def get_pddl_successors(domain_file_name, problem_file_name):
    print('get_pddl_successors with %s and %s'%(domain_file_name,problem_file_name))
    [planning_domain,problem,planning_task] = parse_pddl_files(domain_file_name, problem_file_name)
    init_state = planning_task.initial_state
    pddl_successors = planning_task.get_successor_states(init_state)
    #print('pddl_successors are:')
    #print(pddl_successors)
    return pddl_successors

if __name__ == '__main__' :
    
    [planning_domain,problem]=parse_pddl_files(sys.argv[1],sys.argv[2])
    print()