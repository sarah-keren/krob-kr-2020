...
def parse_list_template(f, iter):
    ...
    for elem in iter:
        #print(elem.contents)
        # SARAH:
        # the parser ignores the oneof and or constructs, since they do not affect the knoweldge sharing
        if 'oneof' in elem.contents or 'or' in elem.contents or 'invariant' in elem.contents :
            continue
...
def parse_problem_def(iter):
    ...
    # SARAH: Added to support contingent planners with 'hidden'
    goal_it = next(iter)
    print(goal_it.contents)
    if ':hidden' in goal_it.contents:
        goal_it = next(iter)
    goal = parse_goal_stmt(goal_it)
    # assert end is reached
    iter.match_end()
