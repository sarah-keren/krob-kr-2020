...
class Operator:
    ...
    def applicable(self, state):
        ...
        @return True if the operator's preconditions is a subset of the state,
                False otherwise
        """
        # SARAH - added support for negative preconditions
        #print('is applicable for op %s'%self.name)
        #print(self.preconditions)
        #print(state)
        #print(self.preconditions <= state)
        for precondition in self.preconditions:
            #print('checking preconditin %s'%precondition)
            positive_sign = True
            if 'False' in precondition.split('=')[1]:
                positive_sign = False
            #print('positive sign %s'%str(positive_sign))
            precondition_content = precondition.split('=')[0]
                
            if positive_sign:
                if not precondition_content in state:
                    #print('no\n\n')
                    return False
            else:
                if precondition_content in state:
                    #print('no\n\n')
                    return False   
    ...
    def applicable_original(self, state):
        """
        Operators are applicable when their set of preconditions is a subset
        of the facts that are true in "state".
    ...
    def get_successor_states(self, state):
        ...
        for op in self.operators:
            if op.applicable(state):
                succ_ops.append([op, op.apply(state)]) 
        return succ_ops
