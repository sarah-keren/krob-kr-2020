...
class FD_Planner : public ClassicalPlanner {
    mutable bool first_call_;
    string planner_config;
    
  public:
    FD_Planner(const KP_Instance &instance, const char *tmpfile_path, const char *planner_path, string planner_config)
      : ClassicalPlanner("FD", tmpfile_path, planner_path, "fd", instance), first_call_(true) {this->planner_config = planner_config; }
    virtual ~FD_Planner();
    virtual int get_raw_plan(const State &state, Instance::Plan &raw_plan) const;
};
