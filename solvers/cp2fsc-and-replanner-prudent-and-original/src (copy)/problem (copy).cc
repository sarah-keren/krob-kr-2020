...
void Instance::write_domain(ostream &os, int indent) const {
    ...
    bool add_cost = false;
    std::size_t foundc = compilation_.find("prud");
    ...
    for( size_t k = 0; k < n_actions(); k++ ) {
        ...
        if( act.effect_.size() + act.when_.size() > 0 )
            act.write(os, indent, *this, add_cost);
    }
...
void Instance::write_problem(ostream &os, const State *state, int indent) const {
   ...
   //START_SARAH
  std::size_t found = compilation_.find("prud");
...
void Instance::Action::write(ostream &os, int indent, const Instance &instance, const bool add_cost) const {
    ...
    action_cost = 1000;   
    size_t found = action_name.find("sensor");	  	
    if (found!=std::string::npos)
    {
      action_type = 1;
      action_cost = 1; //epslion
    }
    found = action_name.find("invariant");	  	
    if (found!=std::string::npos)
    {
      action_type = 2;
      action_cost = 0;	
