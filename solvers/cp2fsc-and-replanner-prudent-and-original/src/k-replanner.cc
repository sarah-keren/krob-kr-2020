...
int main(int argc, char *argv[]) {
	...
	} else if( !skip_options && !strcmp(argv[k], "--compilation") ) {
            compilation = argv[++k];       
    ...
    instance.set_compilation(compilation);	
    ...
    if( translation_type == 0 ) {
        ...
        kp_instance->set_compilation(compilation);	
    ...
    if( g_options.is_enabled("kp:print:preprocessed") ) {
        kp_instance->write_domain(cout);
	std::ofstream outfile_domain ("kp_domain.pddl",std::ofstream::binary);
        kp_instance->write_domain(outfile_domain);

        kp_instance->write_problem(cout);
	std::ofstream outfile_problem ("kp_problem.pddl",std::ofstream::binary);
        kp_instance->write_problem(outfile_problem);
    ...
    cout<<"Classical planner:: "<< opt_planner <<endl;	
    if( opt_planner == "ff" ) {
        ...
        {
	   std::size_t found = opt_planner.find("fd");
           if (found!=std::string::npos)
	   {	
           planner = new FD_Planner(*kp_instance, opt_tmpfile_path.c_str(), opt_planner_path.c_str(),opt_planner);
           }
        } 
