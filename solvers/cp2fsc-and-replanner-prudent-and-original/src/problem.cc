...
void Instance::write_domain(ostream &os, int indent) const {
    ...
    // START_SARAH- add cost support 
    std::size_t foundprud = compilation_.find("prud");
    std::size_t foundkac = compilation_.find("kac");
    if ((foundprud!=std::string::npos)||(foundkac!=std::string::npos))
    {
    // cost function
    os << istr << "(:functions (total-cost))" << endl;
    }	
    ...
    // END_SARAH- add cost support 
    
    // actions
    for( size_t k = 0; k < n_actions(); k++ ) {
        const Action &act = *actions_[k];
        if( act.effect_.size() + act.when_.size() > 0 )
            act.write(os, indent, *this, compilation_);
    }
...
void Instance::Action::write(ostream &os, int indent, const Instance &instance, const std::string compilation) const {

    int action_cost = -1;
    std::size_t foundprud = compilation.find("prud");
    std::size_t foundkac = compilation.find("kac");
    bool add_cost = false;
    if ((foundprud!=std::string::npos)||(foundkac!=std::string::npos))
    {add_cost = true;}
    ...
    {	 
    //START_SARAH
    //Find the type of action (action, sensing or ramification) 
    string action_name = name_->to_string();
    int action_type = 0 ; //  0-action 1-sensor 2-ramification 
    action_cost = 1000;   
    size_t found_sensor = action_name.find("sensor");	  	
    if (found_sensor!=std::string::npos)
    {
      action_type = 1;
      action_cost = 1; //epslion

      // sensing actions are not allowed in the knoweldge acquisition case	
      if(foundkac!=std::string::npos)
      {
        return;
      }	
	
     size_t found_conformant = compilation.find("conformant");	  	
     // sensing actions are not allowed for a conformant agent
      if((foundprud!=std::string::npos)&&(found_conformant!=std::string::npos))
      {		
        return;
      }	

     //Supporting assumptions that cost very high 	
     if (foundprud!=std::string::npos)
     {	

     	size_t found_high = compilation.find("high");
        if (found_high !=std::string::npos)
        {
    	   action_cost = 100000;		
        }
     }//if
    ...
    size_t found_inv = action_name.find("invariant");	  	
    if (found_inv !=std::string::npos)
    {
      action_type = 2;
      action_cost = 0;	
    ...
    size_t found_kac = action_name.find("kac");	  	
    if (found_kac !=std::string::npos)
    {
     action_type = 3;
     size_t found_kac_0 = action_name.find("kac-0");
     size_t found_kac_1 = action_name.find("kac-1");
     size_t found_kac_2 = action_name.find("kac-2");
     action_cost = 1;	

    if (found_kac_0 !=std::string::npos)
    {
	action_cost = 0;	
    }
    if (found_kac_1 !=std::string::npos)
    {
	action_cost = 1;	
    }
    if (found_kac_2 !=std::string::npos)
    {
	action_cost = 100000;	
    }
    ...
    if( n_effects > 0 ) {
       ...
       // START_SARAH- add cost support  	
        if (add_cost) 
        {
          os <<"(increase (total-cost) "<<action_cost<<")"<<endl;
        }
       // END_SARAH- add cost support 
        os << ")";
    }
