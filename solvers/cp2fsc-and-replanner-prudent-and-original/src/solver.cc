...
int Solver::solve(const State &initial_hidden_state,
    ...
    compute_and_add_observations(hidden, state, sensors, sensed, false);
    bool first = true;
	...
	if( status != ClassicalPlanner::SOLVED ) {
   	...
   	if (first){
            compute_and_add_observations(hidden, state, sensors, sensed, true);
	    first = false ;
	}
...
void Solver::compute_and_add_observations(const State &hidden,
                                          State &state,
                                          vector<int> &sensors,
                                          vector<int> &sensed, bool apply_invariants) const {
